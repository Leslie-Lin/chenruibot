#coding=utf8
import re
from method import re_notice,notice,ownthink,calculate,ya,keai,shadiao
re_addnotice_re=re.compile('每.*?提醒我.*', re.I)
re_addnotice=re.compile('.*?提醒我.*', re.I)
re_removenotice_re=re.compile('(删|移)除.*重复提醒', re.I)
re_removenotice=re.compile('(删|移)除.*提醒', re.I)
re_filternotice_re=re.compile('(查看.*重复提醒)',re.I)
...
def main(data):
    if re_addnotice_re.search(data):
        return re_notice.addnotice(data)
    if re_addnotice.search(data):
        return notice.addnotice(data)
    #if re_removenotice_re.search(data):
    #    return re_notice.removenotice(data)
    if re_removenotice.search(data):
        return notice.removenotice(data)
    #if re_filternotice.search(data):
    #    return re_notice.filternotice(data)
    if re_filternotice.search(data):
...
#-----------------傻屌-------------------------------
    ...
    if re_setu.match(data) and qq!=718665461:
